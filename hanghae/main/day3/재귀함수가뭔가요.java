...
public class 재귀함수가뭔가요 {
    ...
    public static String answer(int depth, int N) {
        StringBuilder sb = new StringBuilder();
        if (depth == N) {
            String line = "_".repeat(depth * 4);
            sb.append(line).append("\"재귀함수가 뭔가요?\"\n");
            sb.append(line).append("\"").append("재귀함수는 자기 자신을 호출하는 함수라네").append("\"\n");
