...
public class 소용돌이예쁘게출력하기 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        short[][] drawing = new short[size][size];
        int r = size - 1;
        int c = size - 1;
        int d = 2;
        short number = (short) (size * size);
        ...
        while (number > 0) {
            ...
            if (nextR < 0 || nextC < 0 || nextC >= size || nextR >= size || drawing[nextR][nextC] > 0) {
                d = (d + 1) % 4;
                r = r + drdc[d][0];
                c = c + drdc[d][1];
            } else {
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                int len = String.valueOf(drawing[i + startRowIdx][j + startColIdx]).length();
        ...
        for (int i = 0; i < h; i++) {
            for (int j = 0; j < w; j++) {
                int len = String.valueOf(drawing[i + startRowIdx][j + startColIdx]).length();
                sb.append(" ".repeat(maxLen - len) + drawing[i + startRowIdx][j + startColIdx] + " ");
            }
            sb.append("\n");
        }
